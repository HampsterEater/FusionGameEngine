...
namespace BinaryPhoenix.Fusion.Runtime.Resources
{
	...
	public class PakFile
	{
		...
		public void Load(object url)
		{
            ...
            {
                _pakFileStream = new FileStream(url as string, FileMode.Open, FileAccess.Read);
		...
		public void Save(object url)
		{
            ...
            {
                if (File.Exists(url as string) == false) File.Create(url as string).Close();
                _pakFileStream = new FileStream(url as string, FileMode.Truncate, FileAccess.Write);
                if (_pakFileStream == null) return;
